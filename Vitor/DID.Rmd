...
model <- plm(estimatedTotalAmt ~ treatment + d_inv_df_others, data = pdata, model = "within", effect = "individual")

# Display the results
summary(model)
```
#Year_date	d_inv_df_SBIR_1	df_SBIR_1	d_inv_df_SBIR_2	df_SBIR_2	d_inv_df_STTR_1	df_STTR_1	d_inv_df_STTR_2	df_STTR_2	d_inv_df_PFI	df_PFI	d_inv_df_others	df_others	d_inv	d_59	estimatedTotalAmt	d_99	estimatedTotalAmt_d_99	total_corps	total_fund

```{r}
DID_simple <- plm(total_fund ~ I(total_corps*awardeeCounty) + total_corps + df_others + df_PFI + df_SBIR_1 +df_SBIR_2 + df_STTR_1 + df_STTR_2 + awardeeCounty , data = data, model = "within", effect = "individual")
